...
class json2vsv(object):
    ...
    def _get_content(self,file_name):
        print("INFO:try to update file {} , is exists? {}".format(file_name,os.path.exists(file_name)))
        if os.path.exists(file_name):
            with open(file_name,'r') as f:
                data = f.read()
            if "//progen-spilt:work after here" in data:
                data = data.split("//progen-spilt:work after here")[1].replace("\nendmodule","")
            else:
                raise ValueError("INFO:this file is not generated by progen or loss '//progen-spilt:work after here'")
            return data.strip()
        return ""

    def __call__(self,is_update):
        if is_update:
            ds_content = self._get_content(self.ds_path)
            sv_content = self._get_content(self.tb_path)
        else:
            ds_content = ""
            sv_content = ""
        self.ds_generate(ds_content)
        self.sv_generate(sv_content)
        self.write()
